...
CovidDeath = cbind(WHRData2021[,1:2],WHRData2021[,23])
colnames(CovidDeath) = c('Country', 'Region', 'DeathRate')
...
TitleText = "Average Covid Death per 100000 population for Each Region"
...
baseMEANs = ggplot(CovidDeath, aes(x=reorder(Region,DeathRate, mean),
                             y=DeathRate), ylim=c(0,80)) + theme_classic() + xlab("Region") + ylab("Covid Death per 1M population")
...
plot1= base +  geom_point(aes(colour=factor(Regional.indicator))) + xlim(c(10,50))+ ylim(c(20,70))
plot1

# add the fitted line
plot2 = plot1 + stat_smooth(method='lm', se=F) + theme(legend.title=element_blank())
# plot2 = plot1 + stat_smooth()
plot2
```

```{r}
densePlot = plot2 + facet_wrap(~Regional.indicator,ncol = 1, strip.position = 'right') + theme(legend.position="none") 
densePlot = densePlot + theme(panel.spacing.y = unit(0.1, "lines"),
